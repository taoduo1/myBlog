...
public class ToStringList<T> extends StdConverter<T, List<String>> {
    ...
    public List<String> convert(T in) {
        if (in instanceof Object[])
            return Arrays.asList((Object[]) in).stream().map(o -> toString.convert(o)).collect(Collectors.toList());
        if (in instanceof List)
            return ((List<?>) in).stream().map(o -> toString.convert(o)).collect(Collectors.toList());
